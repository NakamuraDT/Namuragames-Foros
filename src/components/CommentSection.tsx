...
const createComment = async (
    ...
    if (!userId || !author) {
        throw new Error("Necesitas estar verificado para comentar.");
    }
...
export const CommentSection = ({postId}: Props) => {
    ...
    /* Mapa de Comentarios - Organizar Respuestas - Return Tree */
  ...
  <div className="mt-6">
      ...
      {user ? (
        <form onSubmit={handleSubmit}> 
          ...
          className="w-full border border-white/10 bg-transpartent p-2 rounded"/>
          ...
          <button type="submit" disabled={!newCommentText} className="mt-2 bg-purple-500 text-white px-4 py-2 rounded cursor-pointer"> 
            ...
            {isError && <p className="text-red-500 mt-2"> Ups! Hubo un error al publicar el comentario. </p>}
        </form>
     ) : (
        <p className="mb-4 text-gray-600"> Necesitas estar verificado para realizar un comentario. </p>
    )}
